...
#print "line=$line\n";
    ($index_num)=$line=~/^(\d+)\s.*/;
    ($last_word)=$line=~/^\d+\s.*\s+([\w]+)\s*$/;

print "index_num=$index_num last_word=$last_word\n";
 
  #  if(($second)&& ($index_num == 1)){
#	print("****Duplicate index 1 Index1 line=$line\n");
 #       exit -9;
  #  }
   # if ($index_num==1){
#	$second=1;
 ...
 #   }
	...
	while(!(($last_word eq "No") || ($last_word eq "Yes"))){
	    #print "last_word=$last_word, so read in next line\n";
	    $next_line=<>;
            chop($line); 
	    $line=appendToLine($line, $next_line);
	    ($rest, $last_word)=$next_line=~/^(.*)\s+([\w+]+)\s*$/;
	}
